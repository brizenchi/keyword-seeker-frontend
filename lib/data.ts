...
const generateTrendData = (growth: number) => {
  ...
  let currentValue = 20
  for (let i = 1; i <= 30; i++) {
    const change = (Math.random() - 0.2) * (growth / 50) // Biased upwards
...
export const keywordDetails: Record<string, KeywordDetail> = {
  ...
  "Prompt Caching": {
    ...
    growth: 620,
    source: "Google Trends",
    trendData: generateTrendData(620),
    painPoints: [
      {
        title: "Cache Invalidation",
        description: "Knowing when to refresh the cache is difficult.",
      },
      {
        title: "Implementation Complexity",
        description: "KV stores like Redis are overkill for simple caching needs.",
      },
      {
        title: "Hit Rate Visibility",
        description: "No idea if caching is actually saving money.",
      },
    ],
    mvpSuggestion: {
      title: "Middleware for LLMs",
      description: "Drop-in proxy that handles caching automatically.",
